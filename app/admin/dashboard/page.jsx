            ...
            <div style={{marginTop:12}}>
              {products.length === 0 ? <div className="muted">No hay productos</div> : (
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {/* Featured preview and reorder area (mejor UX) */}
                  {(settings.featured && settings.featured.length > 0) ? (
                    <div style={{marginBottom:10}}>
                      <div style={{fontSize:13,fontWeight:700,marginBottom:6}}>Destacados ordenados (arrastrá para reordenar)</div>
                      <div style={{display:'flex',gap:8,overflowX:'auto',padding:6}}>
                        {settings.featured.map((fid, idx) => {
                          ...
                          return (
                            <div key={fid} data-fid={fid} onDragOver={handleFeaturedDragOver} onDrop={(e)=>handleFeaturedDrop(e, idx)} style={{minWidth:160,display:'flex',flexDirection:'column',alignItems:'stretch',gap:6,padding:8,borderRadius:8,background:'#0b0b0b',border:'1px solid #222',boxShadow:'inset 0 1px 0 rgba(255,255,255,0.02)'}}>
                              <div style={{width:'100%',height:80,overflow:'hidden',borderRadius:6,background:'#111'}}>
                              ...
                              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
                                <div style={{display:'flex',alignItems:'center',gap:8}}>
                                  <button draggable onDragStart={(e)=>handleFeaturedDragStart(e, idx)} onTouchStart={(e)=>handleFeaturedTouchStart(e, idx)} onTouchEnd={(e)=>handleFeaturedTouchEnd(e)} className="btn btn-ghost" aria-label="Arrastrar para reordenar" title="Arrastrar" style={{width:30,height:30,display:'inline-flex',alignItems:'center',justifyContent:'center',cursor:'grab',borderRadius:6,background:'#0d0d0d',border:'1px solid #222'}}>
                                    <span style={{fontSize:14,transform:'translateY(1px)'}}>☰</span>
                                  </button>
                                  <div style={{fontSize:13,fontWeight:700}}>{prod.title}</div>
                                </div>
                                {/* Flechas eliminadas: reordenamiento mediante handle o drag */}
                              </div>
                            </div>
                          )
                        })}
              ...
              <div style={{display:'flex',justifyContent:'flex-end',marginTop:12,gap:8}}>
                ...
                <button className="btn btn-primary" onClick={() => {
                  if (imagePickerSelected) {
                    ...
                    nextMain[imagePickerProduct] = imagePickerSelected
                    setSettings({...settings, featuredMain: nextMain})
                  }
