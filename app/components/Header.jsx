    ...
    <header className="sticky top-0 z-50 w-full bg-[#0D0D0D] text-[#EDEDED] font-sans">
      ...
      >
        <nav className="flex flex-col gap-1 px-4 pb-4 max-h-[58vh] overflow-auto" aria-label="Menú móvil">
          <a href="/" onClick={(e)=>{ e.preventDefault(); setOpen(false); setTimeout(()=>router.push('/'),420) }} className="py-3 text-base font-medium transition-colors duration-200 ease-out hover:text-[var(--gold-100)]">Inicio</a>
          <a href="/modelos" onClick={(e)=>{ e.preventDefault(); setOpen(false); setTimeout(()=>router.push('/modelos'),420) }} className="py-3 text-base font-medium transition-colors duration-200 ease-out hover:text-[var(--gold-100)]">Modelos</a>
          <a href="/contacto" onClick={(e)=>{ e.preventDefault(); setOpen(false); setTimeout(()=>router.push('/contacto'),420) }} className="py-3 text-base font-medium transition-colors duration-200 ease-out hover:text-[var(--gold-100)]">Contacto</a>
          {isLoggedIn && (
            <a href="/admin/dashboard" onClick={(e)=>{ e.preventDefault(); setOpen(false); setTimeout(()=>router.push('/admin/dashboard'),420) }} className="mt-2 w-full inline-flex items-center gap-3 rounded-lg btn-panel-mobile px-3 py-3 text-sm font-medium text-[#EDEDED] bg-transparent transition transform-gpu border-t border-white/6 justify-start" aria-label="Ir a Administración">
